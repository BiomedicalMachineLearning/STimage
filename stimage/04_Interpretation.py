...
def clustering(train_adata, path):
    ...
    model_c = LogisticRegression(
        max_iter=10000, penalty='elasticnet', C=0.1, solver='saga', l1_ratio=0.5)
    clf_can_v_non_can = model_c.fit(train_adata.obsm["true_gene_expression"], clustering.fit_predict(
        train_adata.obsm["true_gene_expression"]))
    joblib.dump(clf_can_v_non_can, path +
                'pickle/resnet_block1_log_scaled_relu_clustering_logistic.pkl')
...
def watershed_segment(image):
    ...
    coords = peak_local_max(
        distance, footprint=np.ones((5, 5)), labels=im_fgnd_mask)
    ...
    map_dic = dict(zip(np.unique(im_nuclei_seg_mask),
                   np.arange(len(np.unique(im_nuclei_seg_mask)))))
...
def LIME_heatmap(image_path, explainer, gene, image_fun):
    ...
    explanation = explainer.explain_instance(image, model_predict_gene(
        gene), top_labels=2, hide_color=0, num_samples=1000, segmentation_fn=watershed_segment)
    temp, mask = explanation.get_image_and_mask(
        1, positive_only=False, num_features=10000, hide_rest=True)
