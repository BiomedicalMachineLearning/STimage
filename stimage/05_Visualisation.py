...
def calculate_r(test_adata, prediction_adata, genes):
    ...
    for i, library in enumerate(
        ...
        ad_ = prediction_adata[prediction_adata.obs.library_id ==
                               library, :].copy()
...
def performance_metrics(test_adata, gene_list, path=None):
    ...
    for i in set(test_adata.obs["library_id"]):
        ...
        for j in gene_list:
            score_auroc = roc_auc_score(
                anndata_adata.obsm["predicted_gene_expression"][j], anndata_adata.obsm["true_gene_expression"][j])
    ...
    Performance_metrics = pd.concat([pd.DataFrame(AUROC), pd.DataFrame(
        F1), pd.DataFrame(Precision), pd.DataFrame(Recall)])
    Performance_metrics['Patient'] = list(
        np.repeat(list(set(test_adata.obs["library_id"])), len(gene_list)))*4
    Performance_metrics['Genes'] = gene_list * \
        len(set(Performance_metrics['Patient']))*4
    Performance_metrics['Metrics'] = [
        'AUROC']*len(AUROC)+['F1']*len(F1)+['Precision']*len(Precision)+['Recall']*len(Recall)

    sns.set(font_scale=2, style="whitegrid",)
    plt.figure(figsize=(22.50, 12.50))
    plt.ylim(-0.1, 1.10)
    im = sns.boxplot(x="Patient", y=0, hue="Metrics",
                     data=Performance_metrics, linewidth=3.5, width=0.6)
    ...
    plt.figure(figsize=(22.50, 12.50))
    plt.ylim(-0.1, 1.10)
    im2 = sns.boxplot(x="Genes", y=0, hue="Metrics",
                      data=Performance_metrics, linewidth=3.5, width=0.6)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="STimage software --- Visualisation")
    ...
    spatial_expression_plot = config["RESULTS"].getboolean(
        "spatial_expression_plot")
