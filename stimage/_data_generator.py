...
class DataGenerator_LSTM_one_output(keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        nearest_index = self.get_nearest_index(
            quary_spots, candidates, k_nn=7, leaf_size=self.adata.n_obs//2)
        np.random.shuffle(nearest_index.flat)
        obs_temp_neighbour = self.adata.obs_names[nearest_index.ravel()]

        X_img_neighbour = np.stack([self._load_img(x)
                                   for x in obs_temp_neighbour], axis=0)
        X_img = np.expand_dims(X_img, axis=0)
...
class DataGenerator_LSTM_multi_output(keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        # Generate data
        X_img_final = self._load_neighbour(
            self._load_img, obs_temp, obs_temp_neighbour)
        y_final = self._load_neighbour(
            self._load_label, obs_temp, obs_temp_neighbour)
