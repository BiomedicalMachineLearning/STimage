...
class CLIP_FineTune(pl.LightningModule):
    def __init__(
        self,
        learning_rate=1e-5,
        model_name="resnet50",
        ...
        super().__init__()
        self.learning_rate = learning_rate
        self.BackBone = CLIPModel_CLIP(model_name=model_name, image_embedding=image_embedding)
        self.BackBone.load_state_dict(torch.load(f"./model/CLML-{model_name}_Xenium.ckpt"))
        self.image_encoder = self.BackBone.image_encoder # Image encoder
    ...
    def forward(self, batch):
        ...
        # Calculating Classification loss Cross_Entropy
        pred_prob = self.classifier(image_features)
...
model = CLIP_FineTune(learning_rate=1e-5, 
                      model_name=model_name, image_embedding=image_embedding)
tr_loader = DataLoader(train_dataset, batch_size=128, shuffle=True)
logger = pl.loggers.CSVLogger("./logs", name=f"CLIP-{model_name}-FC-FineTune_Xenium")
trainer = pl.Trainer(accelerator='auto', callbacks=[EarlyStopping(monitor='Validation_loss',mode='min')], 
                     min_epochs=10, logger=logger)
