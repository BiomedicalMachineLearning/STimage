...
test_adata = anndata.read_h5ad(Path+"pickle/test_anndata.h5ad")
...
image_dir = ffpe.obs["tile_path"].tolist()

#########################################################################################################################################
#del image_dir[1315]
#image_dir = random.sample(image_dir,25)
#with open(Path+'SHAP_LIME/image_dir', "wb") as fp:
#    pickle.dump(image_dir, fp)
#########################################################################################################################################

with open(Path+'SHAP_LIME/image_dir', "rb") as fp:
    image_dir = pickle.load(fp)
    
images = []
for i in range(0,len(image_dir)):
    images.append(plt.imread(image_dir[i]).astype('float32'))
...
FFPE_C3_LIMEMask = LIME(image_dir,gene_list[5])
FFPE_C3_LIMEMask_scaled = scale_mask(np.array(FFPE_C3_LIMEMask),25)
np.save(Path+"SHAP_LIME/FFPE_C3_LIMEMask_scaled.npy",FFPE_C3_LIMEMask_scaled)

FFPE_C3_ShapMask = SHAP_global(image_dir,gene_list[5])
FFPE_C3_ShapMask_scaled = SHAP_scale(FFPE_C3_ShapMask,25)
with open(Path+'SHAP_LIME/FFPE_C3_ShapMask_scaled', "wb") as fp:
    pickle.dump(FFPE_C3_ShapMask_scaled, fp)

FFPE_GNAS_LIMEMask = LIME(image_dir,gene_list[9])
FFPE_GNAS_LIMEMask_scaled = scale_mask(np.array(FFPE_GNAS_LIMEMask),25)
np.save(Path+"SHAP_LIME/FFPE_GNAS_LIMEMask_scaled.npy",FFPE_GNAS_LIMEMask_scaled)


FFPE_GNAS_ShapMask = SHAP_global(image_dir,gene_list[9])
FFPE_GNAS_ShapMask_scaled = SHAP_scale(FFPE_GNAS_ShapMask,25)
with open(Path+'SHAP_LIME/FFPE_GNAS_ShapMask_scaled', "wb") as fp:
    pickle.dump(FFPE_GNAS_ShapMask_scaled, fp)
...
for i in range(0,len(image_dir)):
    ...
    explainer = shap.KernelExplainer(model_predict_gene_kernel("GNAS"), np.zeros((1,shap_segments_GNAS_agnostic[i].max())))
...
with open(Path+'SHAP_LIME/shap_segments_GNAS_agnostic', "wb") as fp:
    pickle.dump(shap_segments_GNAS_agnostic, fp)
with open(Path+'SHAP_LIME/shap_values_GNAS_agnostic', "wb") as fp:
...
for i in range(0,len(image_dir)):
    ...
    explainer = shap.KernelExplainer(model_predict_gene_kernel("C3"), np.zeros((1,shap_segments_C3_agnostic[i].max())))
...
with open(Path+'SHAP_LIME/shap_segments_C3_agnostic', "wb") as fp:
    pickle.dump(shap_segments_C3_agnostic, fp)
with open(Path+'SHAP_LIME/shap_values_C3_agnostic', "wb") as fp:
