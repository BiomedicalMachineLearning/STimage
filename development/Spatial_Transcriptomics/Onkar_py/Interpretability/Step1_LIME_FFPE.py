...
test_adata = sc.read_h5ad(Path+"pickle/test_adata_top100_wts.h5ad")

with open('/scratch/project/stseq/Onkar/STimage_v1/pickle/top_100gene.pkl', 'rb') as f:
    gene_list = list(pickle.load(f))


explainer = lime_image.LimeImageExplainer()
clf_resnet = joblib.load(Path+'pickle/LRmodel_100genes.pkl')
...
##########################################################################################################################################
# cord1 = data_FFPE.obs[(data_FFPE.obs['imagecol'] < 8000) & (data_FFPE.obs['imagecol'] > 7000)]
# cord2 = cord1[(cord1['imagerow'] > 10000) & (cord1['imagerow'] < 11500)]
# data_FFPE.obs["active"] = np.where(data_FFPE.obs.index.isin(cord2.index),1,0)
# Selection_region_FFPE_1 = list(data_FFPE.obs[data_FFPE.obs["active"]==1]["tile_path"])

##########################################################################################################################################
# del cord1, cord2

# cord1 = data_FFPE.obs[(data_FFPE.obs['imagecol'] < 17000) & (data_FFPE.obs['imagecol'] > 15000)]
# cord2 = cord1[(cord1['imagerow'] > 7500) & (cord1['imagerow'] < 9500)]
# data_FFPE.obs["active"] = np.where(data_FFPE.obs.index.isin(cord2.index),1,0)
# Selection_region_FFPE_2 = list(data_FFPE.obs[data_FFPE.obs["active"]==1]["tile_path"])

##########################################################################################################################################
# del cord1, cord2

# cord1 = data_FFPE.obs[(data_FFPE.obs['imagecol'] < 9500) & (data_FFPE.obs['imagecol'] > 8500)]
# cord2 = cord1[(cord1['imagerow'] > 8000) & (cord1['imagerow'] < 9500)]
# data_FFPE.obs["active"] = np.where(data_FFPE.obs.index.isin(cord2.index),1,0)
# Selection_region_FFPE_3 = list(data_FFPE.obs[data_FFPE.obs["active"]==1]["tile_path"])
...
# Run LIME
LIMEMask_FFPE_CD24 = LIME_exceptional_handling(Selection_region_FFPE,gene_list[8])
np.save(Path+"pickle/LIMEMask_FFPE_CD24_all.npy",LIMEMask_FFPE_CD24)

LIMEMask_FFPE_CD52 = LIME_exceptional_handling(Selection_region_FFPE,gene_list[85])
np.save(Path+"pickle/LIMEMask_FFPE_CD52_all.npy",LIMEMask_FFPE_CD52)
